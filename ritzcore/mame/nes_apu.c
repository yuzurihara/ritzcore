...
nesapu_device::nesapu_device(const machine_config &mconfig, const char *tag, device_t *owner, UINT32 clock)
	: device_t(NES_APU, clock), //mconfig, NES_APU, "N2A03 APU", tag, owner, clock, "nesapu", __FILE__),
...
void nesapu_device::device_start()
{
	...
	m_samps_per_sync = rate / 60;
	m_buffer_size = m_samps_per_sync;
	m_real_rate = m_samps_per_sync * 60;
	...
	(m_APU.dpcm).memory = machine().m_memory;
	m_stream = machine().stream_alloc(*this, 0, 1, rate, this);
			...
			if (0 == chan->length)
			{
				...
				else
				{
					if (chan->regs[0] & 0x80) /* IRQ Generator */
					{
						...
						set_input_line(N2A03_APU_IRQ_LINE, ASSERT_LINE);
					}
					break;
				}
			}
...
inline void nesapu_device::apu_regwrite(int address, uint8 value)
{
	...
	case APU_WRE0:
		...
		if (0 == (value & 0x80)) {
			...
			set_input_line(N2A03_APU_IRQ_LINE, CLEAR_LINE);
