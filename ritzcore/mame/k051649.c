...
k051649_device::k051649_device(const machine_config &mconfig, const char *tag, device_t *owner, UINT32 clock)
	: device_t(K051649, clock), // mconfig, K051649, "K051649", tag, owner, clock, "k051649", __FILE__),
...
k051649_device::~k051649_device(){
	if(m_mixer_table)
		RitzFree(m_mixer_table);
	if(m_mixer_buffer)
		RitzFree(m_mixer_buffer);
}
...
void k051649_device::device_start()
{
	...
	m_mixer_buffer = (short*)RitzAlloc(2 * m_rate * sizeof(short));
...
void k051649_device::make_mixer_table(int voices)
{
	...
	m_mixer_table = (INT16*)RitzAlloc(sizeof(INT16) * 512*voices);
